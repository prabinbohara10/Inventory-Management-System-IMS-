...
<body id="body">
 ...
 <div class="action_completion" id="action_completion" style="display:none;">book</div>
    ...
    // prepare sql
    $result = $conn->prepare("SELECT * FROM record_table Where status='dd{t gePsf]'");
    $result->execute();
           ...
           echo "<tr id='table_heading'>";
               ...
               while ($row = $result->fetch() ){  
                  ...
                  {echo "<td>".$row[6]."</td>";}
                   ...
                   echo "<td><button id='edit_button' onclick='after_maintenance_clicked'>Edit</button>
...
<div class="div_after_maintenance" id="div_after_maintenance">
  <button class="button2" ><a href="things_not_made_page.php" target="main_iframe">Back</a></button><br>
        <form method="POST" action="things_not_made_page.php" autocomplete="off" >
          ...
          <label for="maintenance_desc">dd{tsf] ljj/)f</label><br>
                <textarea name="maintenance_desc" id="maintenance_desc" size="25"
                 placeholder="dd{tsf] ljj/)f" ></textarea><br>
    ...
    if(isset($_POST["submit_name"]))
{
  ...
  $refrence_work_order_no=$_POST['refrence_work_order_no'];
    ...
    // prepare sql
    $result1 = $conn->prepare("UPDATE record_table SET maintenance_desc='$maintenance_desc', maintenance_date='$maintenance_date', technician='$technician', status='dd{t ePsf]'  
...
setTimeout(function() {
  window.location.href = 'things_not_made_page.php';
}, delayInMilliseconds);
